...
public class MD5 {
    ...
    public static String computeMD5(String message) {
    	...
    	int[] strByte = paddingBytes(message.getBytes());
    ...
    private static int[] paddingBytes(String str) {
    	int length = ((str.length() + 8) / 64) + 1;
    	int[] strByte = new int[length*16];
    	...
    	strByte[length*16-2] = str.length()*8;
	...
	public static String txtMD5(String path) {
		File outputFile = new File("encrypted.txt");
		String result = "";
		try {
			outputFile.createNewFile();
			FileWriter writer;
			writer = new FileWriter("encrypted.txt");
		    ...
		    result = computeMD5(strBuilder.toString());
		    writer.write(result);
			writer.close();
		} catch (IOException e) {
			e.printStackTrace();
